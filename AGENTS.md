...
**Current state: `cms/` is built, `web/` renders the dynamic section.** `cms/` has the properties table, model, factory and seeder, the public `/api/v1/properties` and `/api/v1/health`, and the admin panel in full: session authentication, the dashboard, property CRUD with image replacement and cleanup, the publish toggle, and inline reordering, under a Pest suite that covers every line of `cms/app`. Seed imagery is committed and placed on the disk by the seeder (#27), so `migrate --seed` alone gives a populated CMS. `web/` has the Next.js scaffold, the design system foundation, the layout shell (#13), and Featured Properties reading the real API through `src/lib/api/properties.ts` (#14). The eleven static sections are not built (#15), and neither is the revalidation that closes the loop between the two applications (#16).
