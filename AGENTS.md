...
**Current state: both applications are built and connected.** `cms/` has the properties table, model, factory and seeder, the public `/api/v1/properties` and `/api/v1/health`, and the admin panel in full: session authentication, the dashboard, property CRUD with image replacement and cleanup, the publish toggle, and inline reordering, under a Pest suite that covers every line of `cms/app`. Seed imagery is committed and placed on the disk by the seeder (#27), so `migrate --seed` alone gives a populated CMS. `web/` has the Next.js scaffold, the design system foundation, the layout shell (#13), Featured Properties reading the real API through `src/lib/api/properties.ts` (#14), and the eleven static sections as typed content in `src/content/` (#15). The loop between the two applications is closed by the revalidation callback of #16: `FrontendRevalidator` in the CMS, `src/app/api/revalidate/route.ts` on the frontend. End to end coverage lives in `web/e2e/` (#18): four Playwright scenarios over three frontends, each built against one of the three states the API can be in, because the homepage is prerendered and so that state is settled at build time. The responsive and accessibility pass and the verified README are done too (#17, #19), which leaves nothing outstanding in the plan.
