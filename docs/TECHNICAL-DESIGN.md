...
`cms/README.md` and `web/README.md` are not a second list either, and they are written so that they cannot become one. Each is a pointer: what the application is, the address it answers on, a link to the root README's setup chapter, and the documents that own its topics. Neither carries a command, so neither can disagree with the chapter that does. The Laravel scaffold's own README is what `cms/README.md` replaced, which was 58 lines about the framework, its badges and its contribution guide, and nothing about this project.
...
├── README.md                  setup steps, screenshots, and the index of
│                              this specification. The only setup list in the
│                              repository, per ch. 2.4
...
│   ├── README.md              what this application is, and where its setup
│   │                          and its documents are. No commands: ch. 2.4
...
└── web/                       Next.js application
    ├── README.md              the same pointer, for this application
    ├── .env.example
...
| Next.js end to end | Playwright | The homepage loads and shows cards, the section is hidden on empty data, the fallback appears when the API is down, the mobile navigation opens and closes, and both ways back to the top of the page work and leave focus somewhere real |
