...
describe("Membership", () => {
   ...
   */
  it("takes its emphasis from an ink band, never an accent fill", () => {
    const { container } = render(<Membership tone="ink" />);

    // Full bleed, so the ground is the section itself rather than a card
    // inside it: a contained panel leaves the page running down both sides of
    // the one section meant to interrupt it (DESIGN-SYSTEM ch. 6.11).
